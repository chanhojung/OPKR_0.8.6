...
class SpdctrlRelaxed(SpdController):
    ...
    def update_lead(self, sm, CS, dRel, yRel, vRel, CC):
        ...
        if CS.driverAcc_time and not self.map_decel_only: #운전자가 가속페달 밟으면 크루즈 설정속도를 현재속도+2로 동기화
    ...
    def update_curv(self, CS, sm, curve_speed):
        ...
        if CS.out.cruiseState.modeSel == 1 and Events().names not in [EventName.laneChangeManual, EventName.laneChange] and not (CS.left_blinker_flash or CS.right_blinker_flash)and not self.map_decel_only:
            if curve_speed < 25 and int(CS.clu_Vanz) >= 40 and CS.lead_distance >= 15:
                ...
                wait_time_cmd = 8
            elif curve_speed < 60 and int(CS.clu_Vanz) >= 60 and CS.lead_distance >= 15:
                set_speed = min(65, self.cruise_set_speed_kph - int(CS.clu_Vanz * 0.15))
                self.seq_step_debug = "커브감속-3"
                wait_time_cmd = 8
            elif curve_speed < 75 and int(CS.clu_Vanz) >= 75 and CS.lead_distance >= 15:
                set_speed = min(75, self.cruise_set_speed_kph - int(CS.clu_Vanz * 0.1))
                self.seq_step_debug = "커브감속-2"
                wait_time_cmd = 8
            elif curve_speed < 90 and int(CS.clu_Vanz) >= 90 and CS.lead_distance >= 15:
